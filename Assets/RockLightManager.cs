...
public class RockLightManager : MonoBehaviour
{
    ...
    void Start()
    {
      ...
      for(int i = 0; i < 4; i++)
      {
        ...
        spawnPoints[i] = rocks[i].transform.GetChild(0);
        beams[i] = spawnPoints[i].GetComponent<LineRenderer>();
    ...
    void Update()
    {
      hits[0] = Physics2D.Raycast(spawnPoints[0].position, spawnPoints[0].TransformDirection(Vector3.up));
      hitPoints[0].position = hits[0].point;
      beams[0].enabled = true;
      beams[0].SetPosition(0, spawnPoints[0].position);
      beams[0].SetPosition(1, hitPoints[0].position);

      if(hits[0].collider.name == rocks[1].name)
      {
        hits[1] = Physics2D.Raycast(spawnPoints[1].position, spawnPoints[1].TransformDirection(Vector3.right));
        hitPoints[1].position = hits[1].point;
        beams[1].enabled = true;
        beams[1].SetPosition(0, spawnPoints[1].position);
    ...
    {
      hits[2] = Physics2D.Raycast(spawnPoints[2].position, spawnPoints[2].TransformDirection(Vector3.down));
      hitPoints[2].position = hits[2].point;
      beams[2].enabled = true;
      beams[2].SetPosition(0, spawnPoints[2].position);
    ...
    {
      hits[3] = Physics2D.Raycast(spawnPoints[3].position, spawnPoints[3].TransformDirection(Vector3.left));
      hitPoints[3].position = hits[3].point;
      beams[3].enabled = true;
      beams[3].SetPosition(0, spawnPoints[3].position);
